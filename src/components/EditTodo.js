...
class EditTodo extends Component {
    constructor(props) {
        super(props);
        this.props.dispatch(getTodoById(parseInt(this.props.match.params.id, 10), this.props.todos));
        this.state = {
    ...
    componentWillReceiveProps(props){
        this.setState({text: props.todo.text, date: props.todo.date});
    }
                ...
                <div className="form-group">
                    <div className="input-group">
                        ...
                        <select className="form-control" value={this.state.date}
                            ...
                            {
                                [0, 1, 2, 3, 4, 5, 6].map(i => <option key={i}
                                                                       value={i}>{strings[language]['week_days'][i]}</option>)
                            }
                        </select>
                    </div>
                </div>
                <button type="button" className="btn btn-success w-100" id="confirm"
                        onClick={this.submitForm.bind(this)}>{strings[language]['edit_record']}</button>
            </div>
        )
    }
}
